...
class ChristmasTree(BlenderObject):
    ...
    def __init__(
        ...
        if radius_range[1] < radius_range[0]:
            raise ValueError("The maximum radius of the LEDs must be greater than the minimum radius.")
        ...
        if emission_range[1] < emission_range[0]:
            raise ValueError("The maximum emission of the LEDs must be greater than the minimum emission.")
        ...
        self.local_location = location.copy()
        location.z += height / 2
    ...
    def apply_to_collection(self, collection: bpy.types.Collection) -> None:
        ...
        for i in range(self.n_leds):
            ...
            led_material_output = led_material.node_tree.nodes.new("ShaderNodeOutputMaterial")
